...
private class ScrumSessionController (
    ...
    private fun createSession(@RequestBody sessionData: NewSessionRequest) : Response =
            sessionRepository.save(newSession(sessionData.name)).mapToInfo()

    @GetMapping("/{id}")
    private fun getSessionInfo(@PathVariable("id") sessionId: UUID) : Response =
            sessionRepository.findById(sessionId)
                    .map{ it.mapToInfo() }.orElseGet{ NOT_FOUND_RESPONSE }
...
internal sealed class Response {
    ...
    class SessionInfo (
            val id: UUID,
            val name: String,
            val created: Instant): Response()

    class SessionError (
            val message: String = "Unexpected error"): Response()
